...
接続中のゲストへファイルを送れます。セッションウィンドウにドロップするか、「セッション ▸ ファイルを送る…」
から選びます。セッション中はポインタを捕捉するため掴むものが無く、メニューコマンドを併置しています。
ウィンドウにはドロップ 1 回につき 1 行を、進捗と中止とともに表示し、失敗したファイルは名前を挙げます。
中止は送信を止めるだけで、ゲストが既に受け取ったものは取り返せません。セッションウィンドウに落とした
`.vv` は拒否します。接続チケットを含むためです。接続したいときはランチャーへドロップしてください。
転送は接続に属し、接続が終われば失敗します。方向はホストからゲストのみで、これはエージェントプロトコルの
提供する範囲そのものです。
...
make verify-vendor # Vendor/*.patch を固定した上流に対して再生（ネットワーク必要）
...
（約 120 MB、git 管理外。大半は Xorg が引く Mesa 一式）を作り、SPICE をループバックだけに公開する
コンテナを実行ごとのチケット付きで
...
ゲストの表示をリサイズし、ファイルを送ってゲストが同じ SHA-256 を報告することを要求し、
2 つ目のピアでゲストの無音ストリームから音声再生を受け取ってから、コンテナを停止します。
H.264 と Ravada ポータルは対象外です。
ゲートのゲストは何も描画しない素の X サーバーなので、セッションウィンドウが黒いのは正常です。
`SPICE_CLIENT_LIVE_PEER_DEMO=1 bash Integration/LivePeer/run.sh <env-file>` で起動したピアの
ゲストは、ルートを塗って端末も開きます。手で見るためのもので、ゲートは設定しません。
`make package` は、そのコミットで `make live-peer` が（クリーンなツリーで）合格した記録が無ければ拒否します。
`make verify-vendor` は固定した上流の依存を取得して `Vendor/` のパッチを再生し、パッチが vendored ツリーを
組み立てられなくなったら失敗します。ネットワークが必要なので `make test` には含めません。
...
- `Vendor/SwiftSpice`: v0.4.2 固定版と、クリップボード実アクセス境界、モニタ構成の送信窓、
  ファイル転送の駆動の直列化に関する 3 本の局所パッチ。
