...
3. **2 回目のリサイズは製品の欠陥であり、いまは固定して監視している。** エージェント接続で 2 回目以降の
   ...
   話であり、その局所パッチは ADR-0001 がクリップボード権限に限定しているので、広げるかは別の判断になる。
   それまではゲートがこのモードを `unapplied` として記録し、逆に適用されたら失敗する。直った日に記録を
   更新せず黙って乖離することがないようにするためである。
