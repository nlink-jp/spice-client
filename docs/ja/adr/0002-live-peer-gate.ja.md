...
コンテナがマウントするのは成果物ディレクトリとチケットファイルだけ（どちらも読み取り専用）、
公開はループバックのみ、実行と共に消える（`--rm`、trap による停止、`-no-reboot`）。trap は
SIGKILL やホストのスリープでは走らないため、上限が 2 つある: QEMU は 30 分の `timeout` の下で動き、
