...
サブジェクトが拒否されてピアが生き残ることを示す。フェーズ 2 のエージェント付きゲストは [ADR-0003](0003-agent-guest.ja.md)（2026-09-18）で、
この記録の最小ゲストを、Xorg と `spice-vdagent` も動かすゲストに置き換えた。取得元は upstream の
スクリプトが既定にしている第三者ミラーではなく Alpine 公式 CDN。
...
- 本アプリが参照元と最も異なるクリップボードブローカーとリサイズ経路は、ADR-0003 まで
  シミュレーションのみのままだった。
