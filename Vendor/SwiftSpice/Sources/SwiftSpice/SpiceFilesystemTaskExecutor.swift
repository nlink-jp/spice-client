...
package struct SpiceFilesystemTaskExecutorLimits: Sendable, Equatable {
...
package enum SpiceFilesystemTaskExecutorError: Error, Sendable, Equatable {
    case invalidLimits
...
package enum SpiceFilesystemTaskExecutionError<Failure>: Error, Sendable
where Failure: Error & Sendable {
    case executor(SpiceFilesystemTaskExecutorError)
    case operation(Failure)
}

extension SpiceFilesystemTaskExecutionError: Equatable where Failure: Equatable {}

package struct SpiceFilesystemTaskExecutorDiagnostics: Sendable, Equatable {
...
/// Bounded admission is independent from the GCD-backed Swift executor that
/// runs blocking filesystem calls away from the cooperative executor.
package final class SpiceFilesystemTaskExecutor: Sendable {
    private let taskExecutor = SpiceFilesystemGCDTaskExecutor()
    private let admission: SpiceFilesystemTaskAdmission

    package init(limits: SpiceFilesystemTaskExecutorLimits = .init()) {
        admission = SpiceFilesystemTaskAdmission(limits: limits)
    }

    package func execute<Success: Sendable>(
        retainedByteCount: Int = 0,
        operation: @escaping @Sendable () -> Success
    ) async throws(SpiceFilesystemTaskExecutorError) -> Success {
        let ticket = UUID()
    ...
    package func executeThrowing<Success: Sendable, Failure: Error & Sendable>(
    ...
    ) async throws(SpiceFilesystemTaskExecutionError<Failure>) -> Success {
        let ticket = UUID()
        do {
            try await admission.acquire(ticket: ticket, retainedByteCount: retainedByteCount)
        } catch let error {
        ...
        do {
            ...
            preconditionFailure("typed filesystem operation emitted unexpected error: \(error)")
        }
        if Task.isCancelled {
            await admission.finish(ticket: ticket, outcome: .cancelled)
            throw .executor(.cancelled)
    ...
    package func diagnosticsSnapshot() async -> SpiceFilesystemTaskExecutorDiagnostics {
...
private final class SpiceFilesystemGCDTaskExecutor: TaskExecutor, @unchecked Sendable {
    private let queue = DispatchQueue(
        label: "org.swiftspice.filesystem-task-executor",
        qos: .utility,
...
private actor SpiceFilesystemTaskAdmission {
    ...
    private let limits: SpiceFilesystemTaskExecutorLimits
    ...
    init(limits: SpiceFilesystemTaskExecutorLimits) {
        self.limits = limits
    }
    ...
    func acquire(
    ...
    ) async throws(SpiceFilesystemTaskExecutorError) {
        guard limits.maximumConcurrentJobs > 0,
              limits.maximumPendingJobs >= 0,
              limits.maximumQueuedRetainedBytes >= 0 else {
            reject()
            throw .invalidLimits
        }
        ...
        } catch let error as SpiceFilesystemTaskExecutorError {
            throw error
        } catch {
            throw .cancelled
        }
    ...
    func diagnosticsSnapshot() -> SpiceFilesystemTaskExecutorDiagnostics {
        let activeBytes = currentActiveRetainedBytes
        return SpiceFilesystemTaskExecutorDiagnostics(
    ...
    func close() {
        ...
        for waiter in waiters {
            cancel()
            waiter.continuation.resume(throwing: SpiceFilesystemTaskExecutorError.closed)
        }
    }
    ...
    private func cancelQueued(ticket: UUID) {
        ...
        cancel()
        waiter.continuation.resume(throwing: SpiceFilesystemTaskExecutorError.cancelled)
    }
